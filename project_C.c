...
int main()
{
	...
	printf("Заполняем массив... \nРазмер КВАДРАТНОЙ матрицы: %d на %d \n", m1, m1);
	...
	for (int i = 0; i < m2; i++) {
		for (int j = 0; j < m2; j++) {
			printf("Введите число: ");
			scanf("%d", &matrix_2[i][j]); // зафиксировали значение матрицы matrix_2.
		}
	}
	...
	for (int i = 0; i < m2; i++) {
		for (int j = 0; j < m2; j++) {
			printf("%d ", matrix_2[i][j]); // показали пользователю результат/значение матрицы matrix_2.
		}
		printf("\n");
	}

	printf("\nПеремножим матрицу саму на себя (квадрат матрицы).\n"); // !строка на столбец, строка из первой, столбец из второй.

	int squad_matrix[m2][m2]; // создаём массив для новой матрицы, размер m2 (#define m2 2), сюда запишем рез-т возведения в квадрат матрицы.

	for (int i = 0; i < m2; i++) { // в какую строку (ячейку[0][0]) пишем
		for (int j = 0; j < m2; j++) { // в какой столбец (ячейку[0][0]) пишем
			squad_matrix[i][j] = 0; // обнуляем перед циклом суммирования, ибо если этого не сделать, в ячейках массива может вылезти "мусор" (что угодно из памяти).
			// внутренний цикл умножения строки на столбец
			// памятка, как работает цикл и как суммируются значения: 
			// a = 0 (2x2=4); a = 1 (4+4*6) = 28;
			// a = 0 (2x4=8); a = 1 (8+4*8) = 40;
			// a = 0 (6*2=12); a = 1 (12+8*6) = 60; 
			// a = 0 (6*4=24); a = 1 (24+8*8) = 88;
			for (int a = 0; a < m2; a++) {
